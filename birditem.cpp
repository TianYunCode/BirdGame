...
BirdItem::BirdItem(QGraphicsScene* scene) : m_curflyStatus(0), m_IsLandFall(true), m_IsRaise(true)
{
    ...
    m_birdRefreshTimer->start(10);//间隔为10ms
...
void BirdItem::flyLandfallAnimation()
{
    if(m_IsLandFall)
    {
        ...
        m_flyAnimation->setEndValue(QPoint(pos().x(),m_scene->height()));  //下降一个小鸟高度45
